    ...
    <main className="px-4 py-16 sm:px-6 md:py-24">
      <article className="mx-auto max-w-3xl">
        <nav className="text-sm text-gray-500">
          <Link href="/blog" className="font-medium text-indigo-600 hover:text-indigo-700">
            ← Blog
          </Link>
        </nav>
        <header className="mt-8 border-b border-gray-200 pb-8">
          <p className="text-sm font-medium text-indigo-600">{post.readTime}</p>
        ...
        <div className="mt-10 max-w-none">
          {paragraphs.map((p, i) => (
            <p key={i} className="mb-6 text-base leading-[1.7] text-gray-700">
              {p}
            </p>
          ))}
        </div>
